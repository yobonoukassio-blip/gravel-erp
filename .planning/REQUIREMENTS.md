...
- [ ] **FIN-R01**: Les 4 cost components manquants (extraction, transport, concassage, criblage) ecrivent dans `analytical_entry` via DI dans leur module respectif (plus de hardcoded `0n`)
- [ ] **FIN-R02**: Un @Cron daily (`CostPerTonAggregatorJob`) materialise le cout/tonne par site x calibre a 04:00 UTC sans intervention manuelle
...
- [ ] **FIN-R06**: Le moteur d'alertes (DSH-06) est alimente par des `alert_rule` seedes en migration — au moins 5 regles par defaut (seuil stockpile, spare part, HSE severity>=4, explosifs gap, fuel anomaly)
