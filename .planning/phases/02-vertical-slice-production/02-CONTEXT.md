...
- **D2-83 (Co-design)** : Une session de co-design 2 jours avec 2 opérateurs foreuse + 1 chauffeur camion + 1 chef de quart + 1 HSE est planifiée **AVANT la prod code mobile métier** (note roadmap research flag honorée). Livrable : wireframes validés par les utilisateurs cibles, archivés dans `docs/design/phase-02/`. **Cette session est un livrable Phase 2** (pas un nice-to-have).

### Web (Angular) — extensions Phase 2
- **D2-90 (Modules)** : 7 modules Angular feature lazy-loaded miroirs des modules backend + 1 module `dashboard-site` + 1 module `alerts-inbox`. AG Grid pour listes (rotations, incidents, ravitaillements, événements stockpile/carburant). Formly pour formulaires CRUD (plans forage, seuils stockpile, configuration alertes).
- **D2-91 (Carte site — Leaflet)** : Vue carte centrée sur le polygone du site avec marqueurs : zones d'extraction, bancs, stockpiles, cuves carburant, position dernière des engins (Phase 2 = manuelle / mock ; Phase 5 IoT = télématique réelle). Tiles auto-hébergées OSM ou MapTiler avec fallback offline cache.
