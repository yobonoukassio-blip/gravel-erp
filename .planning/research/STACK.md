# Stack Research — Gravel Ivoire ERP

**Domain:** Multi-tenant ERP for granite quarry / mining operations (multi-site, multi-country West Africa) with offline-first mobile + IoT telematics.
**Researched:** 2026-05-12
**Confidence:** HIGH on backend / DB / mobile / IoT / observability. MEDIUM on identity (depends on operator preference). LOW on accounting export (OHADA/SYSCOHADA tooling is local-vendor territory, not OSS).

## Executive Recommendation

**Prescribed stack** (one sentence per layer, decisions made — not options to weigh):
...
npm install @nestjs/core@^11 @nestjs/common@^11 @nestjs/platform-express@^11 \
  ...
  @nestjs/microservices @nestjs/bull bullmq ioredis \
...
# Web (Angular 20)
ng new gravel-web --strict --routing --style=scss
...
npm install @tanstack/angular-query-experimental \
  ...
  echarts ngx-echarts leaflet @asymmetrik/ngx-leaflet \
  @jsverse/transloco

# Mobile (Flutter)
flutter create gravel_mobile --org ci.gravel --platforms android,ios
flutter pub add powersync drift drift_sqlite_async sqlite_async \
  flutter_riverpod dio flutter_secure_storage \
  geolocator mobile_scanner signature intl \
...
**Phase 1 (MVP — one country, 1-2 sites, ≤500 users):**
...
**Phase 2 (Multi-country, 5-10 sites, real IoT volume):**
...
**Phase 3 (Group consolidation, 20+ sites, regulator-grade audits):**
...
There is **no widely-adopted open-source SYSCOHADA-compliant accounting library** at the level of US/EU equivalents. The pragmatic stack pattern:
...
**This is a domain question to validate with a real Gravel finance director, not a tech question.**
...
---
*Stack research for: Multi-tenant ERP for granite quarry / mining operations*
*Researched: 2026-05-12*
