...
import { SparePartService } from '../../../src/modules/maintenance/services/spare-part.service';

/**
 * Unit tests for SparePartService Phase 8 enhancements (ALT-02).
 * Tests 1-8 from 08-W1-P02-PLAN.md T01 behavior spec.
 *
 * The payload severity is the canonical Phase-8 label ('warning' | 'critical');
 * the boundary mapping warning -> Alert.severity 'high' is applied in
 * AlertsEventHandlers, not here. Stockout comparator is `<= 0` (not `=== 0`)
 * so any negative running balance from concurrent consumes is escalated to
 * 'critical'.
 */
describe('SparePartService', () => {
  ...
  beforeEach(async () => {
    ...
    const mockDataSource = {
      transaction: jest
        .fn()
        .mockImplementation(async (cb: (m: unknown) => unknown) => cb(mockManager)),
    };
  ...
  describe('consume() — threshold_crossed enrichment', () => {
    ...
    it('Test 3b: severity=critical via the `<= 0` comparator (covers negative running balance)', async () => {
      // The `consume()` guard rejects when stock < quantity, so we cannot
      // produce a literal negative result through the public API. The
      // comparator choice is what matters: `<= 0` matches both the 0 case
      // (Test 3) AND any future code path that yields a negative balance
      // (e.g., a concurrent-consume race that bypasses the guard). We
      // co-test the equality branch here and document the intent.
      mockGetOne.mockResolvedValueOnce({
        ...basePart,
        quantityOnHand: 5,
        thresholdMin: 8,
